...
| 4 | Rust+ connectivity core | ConnectionManager (option A/C), pairing listener + consumer, rate limiter/backoff/watchdog, subscriptions/poke, chat priming, server-switch teardown orchestrator, A2S service, device cache hydration | **IN PROGRESS** | ✅ deterministic timing core ported w/ golden tests (14): RateLimiter 50/25s⁻¹/333ms token bucket · TimeoutDetector 5-consecutive + immediate-loss classes incl. cause-chain walk · BackoffPolicy 2s×2→60s · ConnectionCore dual-path proxy fallback w/ legacy German error, per-connection sub/chat state resets, 6s probe/7s cap/2s disconnect budget via injected clock | ⏳ next: rustplus.js transport seam impl, pairing listener, subscriptions/poke orchestrator, server-switch sequence |
