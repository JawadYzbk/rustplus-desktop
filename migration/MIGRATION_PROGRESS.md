...
| 3 | Settings & data stores | versioned stores w/ atomic writes, settings hub (~100 keys), profiles store + safeStorage, backup/restore upgraded crypto, granular reset, legacy migrator M3 + migration UX route | **IN PROGRESS** | ✅ JsonStore primitive: atomic tmp+fsync+rename, schemaVersion, explicit migration hook, corrupt→`corrupt/` quarantine, downgrade protection (9 fs-backed tests) · ✅ storage root locked to `%APPDATA%\RustPlusDesk-Electron` · ✅ first consumer live: ui-prefs.json via `uiPrefs/get|set` IPC w/ debounced renderer persistence (15 IPC+store tests green) |
