...
| 3 | Settings & data stores | versioned stores w/ atomic writes, settings hub (~100 keys), profiles store + safeStorage, backup/restore upgraded crypto, granular reset, legacy migrator M3 + migration UX route | **IN PROGRESS** | ✅ JsonStore primitive (9 fs-backed tests) · ✅ storage root `%APPDATA%\RustPlusDesk-Electron` · ✅ ui-prefs over IPC · ✅ SettingsStore: full ~109-key tracking_settings contract w/ exact legacy defaults, unknown-key rejection, patch+persist (4 tests) · ✅ ProfilesStore: lossless passthrough of unmodeled legacy fields, PlayerToken sealed at rest via SecretCodec seam (safeStorage prod / test codecs), matchKey parity incl. re-pair rotation (5 tests) · ✅ Migrator M3 complete: scan/run over legacy roots (%APPDATA%\RustPlusDesk incl. RustPlusDesktop deaths quirk), permissive settings merge w/ loud warnings, lossless profile import w/ token sealing, verbatim FCM/cache copies, deferred rows for stage-owned data, idempotent re-runs; `/migrate` UX route live (scan → import → report) · ⏳ remaining: backup crypto upgrade (next stage-3 closeout item) |
